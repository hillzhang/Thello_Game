...
public class MoveCheck extends Board {
	...
	private boolean checkNorthRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) {
		...
		while (row >= 0 && !pairCoinFound) // While not beyond the top of the  board and a pair coin has not been found
		{
			...
			if (currentPiece == 0) // If the current square is empty, since oneof your own pieces was not found an overtake isn't possible
			{
				return false;
	...
	private boolean checkNorthWestRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkWestRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkSouthWestRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkSouthRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkSouthEastRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkEastRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	private boolean checkNorthEastRoute(int placedRow, int placedColumn,
			int currentPlayer, int enemy, boolean actualMove) // See north route for how this method works
	{
	...
	// Tests if a move is legal (if actualMove is false, it is the computer
	// testing if there are any possible moves to be made)
	public boolean legalMove(int placedRow, int placedColumn,
			int currentPlayer, boolean actualMove) {
		...
		// Each possible direction is tested and any over takes possible are
		// done. If at least one over take is possible, the move is legal
		if (checkNorthRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkNorthWestRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkWestRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkSouthWestRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkSouthRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkSouthEastRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkEastRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}

		if (checkNorthEastRoute(placedRow, placedColumn, currentPlayer, enemy,
				actualMove)) {
			legalMove = true;
		}
